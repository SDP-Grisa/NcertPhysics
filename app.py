...
def load_resources():
    ...
    return embed_model, text_index, text_chunks, image_index, image_entries, llm, documents, bm25_retriever

embed_model, text_index, text_chunks, image_index, image_entries, llm, documents, bm25_retriever = load_resources()
...
def find_relevant_images(query: str, retrieved_docs, max_images=2):
    ...
    if not relevant:
        ...
        D, I = image_index.search(query_emb, max_images)
        relevant = [image_entries[idx] for idx in I[0] if idx != -1]
...
# Success message
st.success("✅ System Ready: gte-small embeddings • FAISS vector search • BM25 keyword search • Groq LLM")
